...
def compute_grav_pot(pos, planets, thresh):
    if planets.planets_curr_pos.shape == (0,):
        source_pos_arr = np.array([[0,0,0]]).reshape(1,3)
    else:
        source_pos_arr = np.concatenate([planets.planets_curr_pos, np.array([[0,0,0]]).reshape(1,3)])
    r = np.linalg.norm(pos-source_pos_arr)+0.01
    mass_arr = np.array([i.mass for i in planets.planets] + [Constants.MASS_SUN])
    grav_pot = -Constants.G*mass_arr/r
    grav_pot = (np.abs(grav_pot) > thresh)*thresh + (np.abs(grav_pot) < thresh)*grav_pot
